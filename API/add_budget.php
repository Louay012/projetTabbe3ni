...
if($_SERVER['REQUEST_METHOD']=='POST'){
   ...
   try {
        ...
        //cherche si il ya un budget existant pour cette category
        $stmt2 = $pdo->prepare("SELECT * FROM budgets WHERE category_id = :category_id");
        $stmt2->bindParam(':category_id',$category_id);
        $stmt2->execute();
        $existingBudget = $stmt2->fetch(PDO::FETCH_ASSOC);
        if ($existingBudget) {
